...
class TestRunner:
    ...
    def run(self):
        test_suite = TestSuite(tests=[
            makeSuite(TestSigmoid),
            makeSuite(TestLogisticCostFunction),
            makeSuite(TestGradientDescent),
            makeSuite(TestStochasticGradientDescent),
            makeSuite(TestRegularizedLogisticCostFunction),
            makeSuite(TestPrediction),
            makeSuite(TestFMeasure),
...
class TestGradientDescent(TestCase):
    ...
    def test_gradient_descent_func_values(self):
        ...
        epochs = TEST_DATA['opt']['epochs']
        eta = TEST_DATA['opt']['step']
        func_values_expected = TEST_DATA['opt']['func_values']

        _, func_values = gradient_descent(obj_fun, w0, epochs, eta)
...
class TestStochasticGradientDescent(TestCase):
    ...
    def test_stochastic_gradient_descent_func_values(self):
        ...
        _, func_values = stochastic_gradient_descent(logistic_cost_function, x_train, y_train, w0,
                                                     epochs, eta, mini_batch)

        #print(func_values_expected)
